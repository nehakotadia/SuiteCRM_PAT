...
Given('Create Account1: the user navigates to the Accounts tab', async ({ page }) => {
  const accountPage = new AccountPage(page);
  await accountPage.navigateToAccounts();
});

Given('the user clicks the Create Account button1', async ({ page }) => {
  const accountPage = new AccountPage(page);
  await accountPage.clickCreateAccount();
});

When('the user clicks Save button after entering valid information in all mandatory fields1', async ({ page }) => {
...
Then('the account should be created successfully1', async ({ page }) => {
  const accountPage = new AccountPage(page);
